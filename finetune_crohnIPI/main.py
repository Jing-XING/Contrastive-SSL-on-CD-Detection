...
def train(net, train_iter, test_iter, criterion, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        print('epoch %d, train loss %.4f, train acc %.3f, test loss %.4f, test acc %.3f, best acc %.3f, time %.1f sec'
              % (epoch + 1, train_loss_sum / batch_count, train_acc_sum / n, test_loss_sum / batch_count2, test_acc_sum / n2,
                 best_accuracy, time.time() - start))
        
mode = 'train'
foldnum=1
finetune_model='crohnIPI_10epochs_fold1'
load_path = '../reference_code/BYOL-PyTorch-yaox12-/ckpt/byol_crohnIPI/275_first_saved/resnet34_10.pth.tar'
...
log_path = os.path.join(log_path,finetune_model,now)
ckpt_path= os.path.join('ckpt',finetune_model)
...
pretrained_net = load_dic(pretrained_net, load_path)
